#!/usr/bin/env python3
"""
RQ2: Lambda Sweep for Baseline Policies (Greedy and PowerCool)
Based on RQ1's run_policy_comparison_nominal.py with added lambda sweep capability
"""
...
def run_policy_experiment(config_file, policy_name, job_bundle_dir, lambda_val, seed=42):
    """Run a single experiment for a policy with specified lambda and seed."""
    ...
    if policy_name == "Greedy":
        policy = create_policy("greedy_capacity", env)
    elif policy_name == "PowerCool":
        policy = create_policy("power_cooling", env)
    else:
    ...
    print(f"      λ={lambda_val:.1f}: Completion={completion_rate:.1f}%, Util={np.mean(cpu_utils):.1f}/{np.mean(gpu_utils):.1f}%, Energy={energy_per_job:.3f} kWh/job")

    return results


def main():
    parser = argparse.ArgumentParser(description='RQ2: Lambda Sweep for Baseline Policies')
    ...
    log_file = os.path.join(logs_dir, f'rq2_baseline_lambda_sweep_{timestamp}.log')
    logging.basicConfig(
        level=logging.INFO,
    ...
    logging.info("Starting RQ2 Baseline Lambda Sweep Experiment")
    logging.info(f"Config: {config_file}")
    logging.info(f"Bundle: {bundle_dir}")

    # Define experiment parameters
    policies = ["Greedy", "PowerCool"]
    ...
    print(f"Total experiments: {len(policies)} × {len(lambda_values)} × {n_seeds} = {len(policies) * len(lambda_values) * n_seeds}")
    print("=" * 80)

    all_results = []
    experiment_count = 0
    total_experiments = len(policies) * len(lambda_values) * n_seeds
    ...
    for lambda_val in lambda_values:
        ...
        for policy_name in policies:
            ...
            for seed in range(n_seeds):
                ...
                try:
                    result = run_policy_experiment(config_file, policy_name, bundle_dir, lambda_val, seed)
                    policy_results.append(result)
                    all_results.append(result)
                    logging.info(f"Completed {policy_name} λ={lambda_val} seed={seed}")
                except Exception as e:
                    print(f"      ❌ Error: {e}")
                    logging.error(f"Failed {policy_name} λ={lambda_val} seed={seed}: {e}")

            # Print average for this policy at this lambda
            if policy_results:
                avg_completion = np.mean([r['completion_rate'] for r in policy_results])
                avg_cpu_util = np.mean([r['cpu_util_mean'] for r in policy_results])
                avg_gpu_util = np.mean([r['gpu_util_mean'] for r in policy_results])
                avg_energy = np.mean([r['energy_per_job'] for r in policy_results])
                print(f"    Average: Completion={avg_completion:.1f}%, Util={avg_cpu_util:.1f}/{avg_gpu_util:.1f}%, Energy={avg_energy:.3f} kWh/job")

    # Save results
    df = pd.DataFrame(all_results)
    results_file = os.path.join(results_dir, f'rq2_baseline_lambda_sweep_{timestamp}.csv')
    ...
    summary = df.groupby(['policy', 'lambda']).agg({
    ...
    # Save summary
    summary_file = os.path.join(results_dir, f'rq2_baseline_lambda_summary_{timestamp}.csv')
